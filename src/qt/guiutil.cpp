...
{
    // return if URI is not valid or is no bitgoldencoin: URI
...
{
    // Convert bitgoldencoin:// to bitgoldencoin:
    //
    //    Cannot handle this later, because bitgoldencoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("bitgoldencoin://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "bitgoldencoin:");
    }
...
{
    QString ret = QString("bitgoldencoin:%1").arg(info.address);
    int paramCount = 0;
